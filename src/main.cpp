    ...
    while (window.isOpen())
    {
        ...
        if(state_game == true)
        {
            ...
            for(size_t x = 0; x < balls.size(); x++)
            {
                ...
                if(balls[x].pos.x + balls[x].vel.x < 10)
                {
                    balls[x].pos.x += abs(balls[x].vel.x) * 2;
                    balls[x].vel.x =- balls[x].vel.x;
                }
                if(balls[x].pos.x + balls[x].vel.x > 790-balls[x].size_ball.x)
                {
                    balls[x].pos.x -= abs(balls[x].vel.x) * 2;
                    balls[x].vel.x =- balls[x].vel.x;
                }
                if(balls[x].pos.y + balls[x].vel.y < 10)
                {
                    balls[x].pos.y += abs(balls[x].vel.y) * 2;
                    balls[x].vel.y =- balls[x].vel.y;
                }
                /**< bottom wall */
                if(balls[x].pos.y + balls[x].vel.y > 600 - balls[x].size_ball.y)
                {
                    balls[x].pos = sf::Vector2f(pos_desk.x + 32, pos_desk.y - 32);
                    balls[x].vel = sf::Vector2f(balls[x].vel.x,-5);
                ...
                /**< collision with desk */
                if(isCollision(balls[x].pos.x + balls[x].vel.x, balls[x].pos.y + balls[x].vel.y,
                                balls[x].size_ball.x, balls[x].size_ball.y,
                                pos_desk.x + delta_desk.x, pos_desk.y,
                                size_desk.x, size_desk.y))
                {
                    float temp_add_vel = 1.2f;

                    balls[x].vel.y = -(balls[x].vel.y * temp_add_vel);
                    balls[x].pos.y -= abs(balls[x].vel.y) * 2;
                }

                if(balls[x].vel.y > 6)
                    balls[x].vel.y = 6;
                else if(balls[x].vel.y < -6)
                    balls[x].vel.y = -6;

                if(balls[x].vel.y > 0 && balls[x].vel.y < 3)
                    balls[x].vel.y++;
                else if(balls[x].vel.y < 0 && balls[x].vel.y > -3)
                    balls[x].vel.y--;

                balls[x].particle_emitter.setPosition(balls[x].pos.x + balls[x].size_ball.x / 2 - 1,
                                                      balls[x].pos.y + balls[x].size_ball.y / 2 - 1);
                balls[x].particle_emitter.setVelocityParticles((rand()%2-1),(rand()%2-1));
                balls[x].particle_emitter.update();

                balls[x].pos += balls[x].vel;
            }
            ...
            for(size_t x = 0; x < blocks.size(); x++)
            {
                for(size_t i = 0; i < balls.size(); i++)
                {
                    ...
                    if(isCollision(balls[i].pos.x + balls[i].vel.x, balls[i].pos.y + balls[i].vel.y,
                                   balls[i].size_ball.x, balls[i].size_ball.y,
                                   blocks[x].x, blocks[x].y, blocks[x].wx, blocks[x].hy))
                    {
                        if(balls[i].pos.x + balls[i].size_ball.x < blocks[x].x)
                            balls[i].vel.x =- balls[i].vel.x;
                        if(balls[i].pos.x > blocks[x].x+32)
                            balls[i].vel.x =- balls[i].vel.x;
                        if(balls[i].pos.y + balls[i].size_ball.y < blocks[x].y)
                            balls[i].vel.y =- balls[i].vel.y;
                        if(balls[i].pos.y > blocks[x].y+32)
                            balls[i].vel.y =- balls[i].vel.y;
            ...
            for(size_t x = 0; x < balls.size(); x++)
            {
                spr_ball.setPosition(balls[x].pos);
                for(int i = 0; i < balls[x].particle_emitter.getNumberOfParticles(); i++)
                {
                    spr_particle.setPosition(balls[x].particle_emitter.getParticle(i).position.x-4,
                                             balls[x].particle_emitter.getParticle(i).position.y-4);
                    spr_particle.setTextureRect(sf::IntRect(0,0,8,8));
